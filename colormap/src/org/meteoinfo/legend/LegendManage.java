...
public class LegendManage {
     ...
     * @return 图例数组
     */
    public static double[] getValues(double min, double max, int num,int index) {
        java.util.List<Double> list = new ArrayList();
        java.util.List<Double> list1 = new ArrayList();
        ...
        if(index!=0){
            ...
            double[] values = null;
            if((max-min)/num>1){
                values = new double[num-1];
                for (int i = 0; i < num-1; i++) {
                    values[i] = (int)(min+(i+1)*(max-min)/num);
                }
            }else if((max-min)/num<=1&&(max-min)/num>0){
                values = new double[(int)(max-min)];
                for (int i = 0; i < (int)(max-min); i++) {
                    values[i]=(int) (max-min+i+1);
                }
            }else{
                values = new double[1];
                values[0]=(int)min;
        ...
        double avg = (max - min) / num;
        for (int i = 0; i < num; i++) {
            double temp = min + avg * (i + 1);
            /*values[i] = Double.valueOf(df.format(temp));*/
            list.add(Double.valueOf(df.format(temp)));
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if(list1.size()!=0){
                ...
                for(int k=0;k<list1.size();k++){
                    if(Double.valueOf(list1.get(k))-Double.valueOf(temp)==0){
                        flag = true;
                    }
                    if (flag){
                        break;
